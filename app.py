...
# ==========================================
# 3. منطق شمال الإسماعيلية (التركيز هنا)
# ==========================================
...
def process_file_final(file_path, filename):
    try:
        # قراءة أولية للبحث عن الهيدر
        df_temp = pd.read_excel(file_path, header=None)
        start_row = 0
        found_header = False
        
        # وسعت نطاق البحث لـ 50 سطر عشان الملفات "الرخمة"
        ...
        if col_name:
            ...
            # --- تحديد اسم الهندسة (تعديل جذري) ---
            fname_clean = filename.replace('أ', 'ا').replace('ة', 'ه').lower()
            
            # منطق قوي جداً للتعرف على إسماعيلية ثان
            ...
            # أي ملف يحتوي على "ثان" أو "2" أو "تاني" سيتم اعتباره إسماعيلية ثان
            elif 'ثان' in fname_clean or '2' in fname_clean or 'تاني' in fname_clean or 'ثانى' in fname_clean: dist = 'إسماعيلية ثان'
            else: dist = 'غير محدد' # سيظهر في الرسم كـ "غير محدد" لنعرف أن الملف قرئ ولكن الاسم غريب
...
if page == "المحطات العامة":
    ...
    # --- جزء الديباج (Debug) لكشف المستور ---
    with st.expander("🛠️ فحص الملفات (اضغط هنا لو فيه مشكلة)", expanded=False):
        files_found = [f for f in os.listdir('.') if f.endswith(('.xls', '.xlsx'))]
        st.write(f"📂 الملفات الموجودة في المجلد: {files_found}")
    ...
    for f in os.listdir('.'):
        if f.endswith(('.xls', '.xlsx')) and f not in excluded and "517" not in f and not f.startswith('~$'):
            ...
            if res is not None:
                ...
                # رسالة خطأ تظهر فقط لو الملف فيه مشكلة
                st.warning(f"⚠️ تم تجاهل الملف '{f}': {msg}")
    ...
    if all_dfs:
        ...
        st.error("❌ لم يتم العثور على أي ملفات صالحة للقراءة! تأكد من رفع ملفات الإكسل بجوار ملف app.py")
